...
fn json_lines_process_handles_ping_repair_and_position_lifecycle() {
    ...
    let requests = [
        ...
        json!({"id": 2, "method": "positions.count"}),
        json!({"id": 3, "method": "positions.repair"}),
        json!({"id": 4, "method": "positions.save", "params": {"guide_key": "1:90071992547409931234", "scroll_top": 12.5}}),
        json!({"id": 5, "method": "positions.get", "params": {"guide_key": "1:90071992547409931234"}}),
        json!({"id": 6, "method": "positions.snapshot"}),
        json!({"id": 7, "method": "positions.delete", "params": {"guide_key": "1:90071992547409931234"}}),
        json!({"id": 8, "method": "positions.clear"}),
        json!({"id": 9, "method": "hotkey.status"}),
        json!({"id": 10, "method": "guides.get", "params": {"guide_id": "0", "force_refresh": false}}),
        json!({"id": 11, "method": "guides.get_cached", "params": {"guide_id": "3414883877"}}),
        json!({"id": 12, "method": "guides.clear"}),
        json!({"id": 13, "method": "guides.stats"}),
        json!({"id": 14, "method": "images.get", "params": {"url": "http://evil.example/image.png", "allow_download": false}}),
        json!({"id": 15, "method": "images.get", "params": {"url": "https://images.steamusercontent.com/ugc/example/image.png", "allow_download": false}}),
        json!({"id": 16, "method": "images.clear"}),
        json!({"id": 17, "method": "reader_cache.stats"}),
    ];
    {
    ...
    assert_eq!(responses[&5]["result"]["scroll_top"], 12.5);
    assert_eq!(
        responses[&6]["result"]["1:90071992547409931234"]["scroll_top"],
        12.5
    );
    assert_eq!(responses[&7]["result"], true);
    assert_eq!(responses[&8]["result"], 0);
    assert_eq!(responses[&9]["result"]["button"], "L4");
    assert_eq!(responses[&9]["result"]["running"], true);
    assert_eq!(responses[&10]["error"]["kind"], "validation");
    assert_eq!(responses[&11]["result"], Value::Null);
    assert_eq!(
        responses[&12]["result"],
        json!({"bytesRemoved": 0, "filesRemoved": 0})
    );
    assert_eq!(responses[&13]["result"], json!({"bytes": 0, "files": 0}));
    assert_eq!(responses[&14]["error"]["kind"], "validation");
    assert_eq!(responses[&15]["result"], Value::Null);
    assert_eq!(
        responses[&16]["result"],
        json!({"bytesRemoved": 0, "filesRemoved": 0})
    );
